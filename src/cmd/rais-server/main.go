...
func main() {
	...
	// Build an image handler per configured IIIF spec version.  An empty web path
	// disables that version; config validation guarantees at least one is set.
	var handlers []*ImageHandler
	for _, vc := range []struct {
		version iiif.Version
		path    string
	}{
		{iiif.V2, viper.GetString("IIIFWebPathV2")},
		{iiif.V3, viper.GetString("IIIFWebPathV3")},
	} {
		if vc.path == "" {
			Logger.Infof("IIIF v%s endpoint disabled (no web path configured)", vc.version)
			continue
		}
		webPath := path.Clean(vc.path)
		if webPath != vc.path {
			Logger.Warnf("WebPath %q cleaned; using %q instead", vc.path, webPath)
		}

		ih := NewImageHandler(tilePath, webPath, vc.version)
		...
		Logger.Infof("Serving IIIF v%s on %q", vc.version, webPath)
