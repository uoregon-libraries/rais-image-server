...
func NewImageTile(jp2 *JP2Image, r image.Rectangle, width, height int) (*ImageTile, error) {
	...
	if C.opj_set_decoded_resolution_factor(jp2.codec, C.OPJ_UINT32(level)) == C.OPJ_FALSE {
	...
	if C.opj_set_decode_area(jp2.codec, jp2.image, C.OPJ_INT32(r.Min.X), C.OPJ_INT32(r.Min.Y), C.OPJ_INT32(r.Max.X), C.OPJ_INT32(r.Max.Y)) == C.OPJ_FALSE {
		return nil, errors.New("failed to set the decoded area")
	}

	if C.opj_decode(jp2.codec, jp2.stream, jp2.image) == C.OPJ_FALSE {
		return nil, errors.New("failed to decode image")
	}
	if C.opj_end_decompress(jp2.codec, jp2.stream) == C.OPJ_FALSE {
		return nil, errors.New("failed to decode image")
	}
